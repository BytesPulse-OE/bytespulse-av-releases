...
if [ -d "$HESTIA_WEB" ]; then
    ...
    rm -f  "$HESTIA_WEB/list/bpav-admin/index.php"
    rm -f  "$HESTIA_WEB/templates/pages/list_bpav_admin.php"
    rmdir  "$HESTIA_WEB/list/bpav-admin" 2>/dev/null || true
...
# Clean up mailer from older installs
rm -f /etc/bpav/bpav-mailer.php
rm -f /etc/cron.d/bpav-mailer
rm -f /etc/sudoers.d/bpav-mail
rm -rf /etc/bpav/hestia-plugin

# ── Remove background mailer ───────────────────────────────────────────
rm -f /etc/bpav/bpav-mailer.php
rm -f /etc/cron.d/bpav-mailer
rm -f /etc/sudoers.d/bpav-mail
info "Removed mailer cron and script"
