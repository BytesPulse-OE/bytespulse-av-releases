...
mkdir -p "$CONFIG_DIR" "$LOG_DIR" "$RUN_DIR" "$CONFIG_DIR/i18n"
# /etc/bpav holds secrets (config.json, admin_token.txt) — keep it closed.
chmod 750 "$CONFIG_DIR" "$LOG_DIR"
chmod 755 "$RUN_DIR"
# Translations are not secret and the HestiaCP panel (user "hestiaweb") must
# read them. Grant traverse on /etc/bpav plus read on the i18n subdir only;
# the secret files below stay 0600 so they remain unreadable.
chmod 751 "$CONFIG_DIR"
chmod 755 "$CONFIG_DIR/i18n"
...
# Fix socket permissions so the HestiaCP panel (hestiaweb) can connect
ExecStartPost=/bin/sh -c 'sleep 1 && chown root:hestiaweb /run/bpav/bpav.sock && chmod 660 /run/bpav/bpav.sock'
Restart=on-failure
RestartSec=5
...
CPUQuota=40%
MemoryMax=512M
# ProtectSystem must be off — daemon needs to read/write /home/* and /etc/bpav
ProtectSystem=false
ReadWritePaths=/etc/bpav /var/log/bpav /run/bpav /home /tmp
...
if [ ! -d "/usr/local/hestia" ]; then
    ...
    warn "To install plugin later: download plugin.zip from the release and run: sudo bash install.sh"
else
    rm -rf "$PLUGIN_TMP"
    mkdir -p "$PLUGIN_TMP"
    ...
    if curl -fsSL --retry 3 \
        ...
        warn "Could not download plugin.zip — install manually after release"
        rm -rf "$PLUGIN_TMP"
    fi
fi
